	...
	new Vue({
		...
		methods: {
			...
			getList: function () {
				...
				var url = baseUrl+"manage/comment_report/find_paged_infos.json";
			...
			handleArtDel: function (index, row) {
				this.$confirm('确认删除该记录吗？', '提示', {
					...
					var self = this;
					this.listLoading = true;
					var url = baseUrl+"manage/comment/update.json";
					ajaxReq(url, {token: token, uid: uid, pid: row.commentId, isDelete: 1 }, function(res){
						self.listLoading = false;
						if(res.code > 0){
							self.$message({
								message: '删除成功',
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							var url = baseUrl+"manage/comment_report/update.json";
							var params = {
									token: token,
									uid: uid, 
